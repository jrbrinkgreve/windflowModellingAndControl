...
function params = load_params()
    ...
    params.alpha_gradient = 0.05;% deg/m
...
params = load_params();
[X, Y] = ndgrid(params.xrange, params.yrange); %X and Y should be of the same shape, same # entries
...
%note to self: == 0 if input angle == 0
...
if gamma < params.angle_tolerance    %to prevent numerical instability, even though chi * c_i --> as gamma --> 0
    chi = 0;
...
chi = 1 ./ (params.lambda * sin(gamma));   %NOTE this gives NaN problems in case of no yaw / veer
...
ratio = (params.R.^2 .* params.CT .* cos(gamma).^3) ./ (2 .* sigma_hat_squared); %or sigma_hat_squared here?
...
if params.numzpoints > 10
    disp("error: too many zpoints for plotting")
else
for z_idx = 1:params.numzpoints
figure('Color','w');
...
title(['Normalized velocity deficit \Deltau./u_h (relative yaw = ', num2str(rad2deg(params.beta + params.alpha(z_idx))), '°), veer gradient = ' , num2str(params.alpha_gradient), '°/m']);
